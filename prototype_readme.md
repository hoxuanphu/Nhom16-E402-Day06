# Prototype — EduBot AI

## Mô tả
Phụ huynh/học sinh gửi câu hỏi mới trong chat inbox → hệ thống truy xuất FAQ + policy hiện hành → AI sinh nháp câu trả lời có trích nguồn. Tư vấn viên quyết định chọn câu trả lời của bot hoặc sửa câu trả lời của chatbot.

## Level: Mock prototype
- UI build bằng Antigravity (HTML/CSS/JS)

## Links


## Tools
- UI: Antigravity
- AI: Google Gemini 3.1 Pro

## Phân công
| Thành viên | Phần | Output |
|-----------|------|--------|
| Hồ Xuân Phú - 2A202600061 |||
| Lại Đức Anh - 2A202600374 | Mini Spec final | spec_final.md |
| Phạm Anh Quân - 2A202600252 | RAG Pipeline Developer | spec-final.md phần 3 |
| Đào Danh Đăng Phụng - 2A202600358 |||
| Nguyễn Minh Trí - 2A202600182 | ROI 3 kịch bản | spec-final.md phần 5 |
| Hoàng Ngọc Thạch - 2A202600068 | Canvas + UI prototype | spec-final.md phần 1, prototype/ |
